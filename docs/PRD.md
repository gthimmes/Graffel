...
- **Shipped:** v3.22 GCP + Azure packs — 20 services each, same opt-in off-ramp, each in a visually distinct idiom (AWS = filled category tiles, GCP = white tiles with brand-colored glyphs, Azure = filled Azure-blue tiles). Full vendor cloud coverage for architects who opt in.
- **Shipped:** v3.23 Walkthrough (presenter) mode — the depth differentiator's payoff (see [ADR-0013](./adr/0013-walkthrough-mode.md)): author ordered "stops" (each captures a drill-down level + selection + a note) in a 🎬 Present side panel, then step through them full-screen — Prev/Next, a step counter, arrow-key navigation, and the camera framing each stop's highlighted shapes. Tours persist with the document (an additive `presentation` block, no schema bump) so a shared diagram can be presented read-only. Closes the "presenter walkthrough" fast-follow from v3.14.
- **Shipped:** v3.24 Mermaid interop — two-way conversion for the Mermaid `graph`/`flowchart` dialect (see [ADR-0014](./adr/0014-mermaid-interop.md)). **Import:** paste a flowchart in the `⇄ Mermaid` dialog (shapes `[]`/`()`/`{}`/`(())`/`[()]`, edge + inline labels, chains, comments) → it opens as a new document, auto-laid-out by the existing ELK pass. **Export:** the level in view serializes back to `graph` text (Copy / Download `.mmd`) for READMEs, wikis, and ADRs. Pure parse/build/serialize stages with a thin impure orchestrator; structure round-trips faithfully (styling/geometry are necessarily lossy). Removes the biggest first-run barrier — getting existing diagrams in. Flat-only for v1; `subgraph`↔container mapping is the next fast-follow.
- **Shipped:** v3.25 Mermaid `subgraph` ↔ drill-down container — the depth-interop payoff ([ADR-0014 §Update](./adr/0014-mermaid-interop.md)). Nested architectures now round-trip: a Mermaid `subgraph` imports as an enterable container holding its members, and a container exports as a `subgraph` block. Layout uses **native ELK hierarchy** (`elk.hierarchyHandling: INCLUDE_CHILDREN`) — ELK sizes each container to fit its children and returns parent-relative coordinates, which is exactly Graffel's storage model, so no coordinate conversion. Export now serializes the whole subtree under the viewed level. Imports are ordinary nested documents, so drill-down + presenter + `.graffel` export apply unchanged.
- **Shipped:** v3.26 Version history + autosave trust — closes the "did my work save?" / "can I get back?" gap (see [ADR-0015](./adr/0015-version-history.md)). A toolbar **save indicator** ("Saving…" → "✓ Saved · 2s ago") makes autosave visible; a **🕘 History panel** lists restorable snapshots — time-gated **auto-checkpoints** (piggybacking autosave, ~1 per 90s of editing) plus named **manual** snapshots. **Restore is non-destructive** (it checkpoints the current state first, so a restore is itself reversible). Snapshots are full documents in a per-document localStorage key, capped (manuals never dropped for the auto cap); pruning + gating are pure/tested. Additive — no schema change, nothing to migrate.
- **Shipped:** v3.27 Living diagrams — docker-compose → diagram (generate) — the first slice of *diagrams that don't rot* (see [ADR-0016](./adr/0016-living-diagrams.md)). Point Graffel at a `docker-compose.yml` (🐳 Compose / paste / file) and it generates an auto-laid-out architecture: services become nodes, `depends_on` becomes the wiring, and the **image is inferred into a real shape** (Postgres→database, Redis→cache, RabbitMQ/Kafka→queue, Nginx/Traefik→gateway, Prometheus/Grafana→monitoring, else a generic service). Ids are **source-derived** (`compose:<service>`) and the source text is stored as document **provenance** (`metadata.source`, carried through the store so it survives autosave) — the foundation for **re-sync** (slice 2). Pure parse/infer/build are unit-tested; ELK lays out. Reuses the Mermaid import spine.
- **Shipped:** v3.28 Living diagrams — re-sync (the "doesn't rot" payoff) ([ADR-0016 §Update](./adr/0016-living-diagrams.md)). Re-importing an updated `docker-compose.yml` now **merges into the existing diagram** instead of replacing it: services you moved/restyled keep their layout, new ones are staged below the current content, removed ones drop out (with their edges), and an image swap re-types the shape. **Hand-drawn nodes/edges are never touched.** Matching is by the slice-1 stable ids; the merge is a pure, unit-tested function (`syncComposeGraph`). Applied as **one undoable step** (new `replaceGraph` store action); provenance advances so the next re-sync diffs against the latest file. The dialog shows a **change report** ("+2 added · −1 removed · N unchanged · your layout was preserved").
